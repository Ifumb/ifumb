...
export const BRIDGE_BUTTONS_HEIGHT = 52

/** A member node's full height, wider than `MEMBER_NODE_SIZE.height` once it has a bridge to show. */
export function memberNodeHeight(bridgeLinkCount: number, hasPivot = false): number {
  return (
    MEMBER_NODE_SIZE.height +
    bridgeLinkCount * BRIDGE_BUTTONS_HEIGHT +
    (hasPivot ? PIVOT_BUTTON_HEIGHT : 0)
  )
}

// reason: une cible de 44 px reste supérieure à 24 px au zoom initial minimal de 0,6.
export const PIVOT_BUTTON_HEIGHT = 48
