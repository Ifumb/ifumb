...
export type ManageableFamily = { readonly tree: Tree; readonly family: Family }

export type FamilyDeps = { readonly trees: TreeReader; readonly families: FamilyReader }

/** The family of a tree whose unions the viewer may manage. */
export async function manageableFamily(
  ...
  if (!canManageUnions(access.value.role)) return err({ kind: 'UNION_MANAGEMENT_FORBIDDEN' })

  const { tree } = access.value.listing
  return ok({ tree, family: await deps.families.loadFamily(tree.id) })
}
