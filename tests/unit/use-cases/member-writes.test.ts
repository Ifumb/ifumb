...
describe('member creation and update', () => {
  ...
  describe('CreateMemberUseCase', () => {
    it('creates a member and its MEMBER_CREATED entry in one transaction', async () => {
      ...
      expect(result).toEqual({ ok: true, value: { memberId: 'usr_1' } })
    ...
    it.each([
      ...
      [{ treeId: 'tree_diallo', viewerId: EDITOR_ID }, 'MEMBER_MANAGEMENT_FORBIDDEN'],
  ...
  describe('UpdateMemberUseCase', () => {
    ...
    it('writes nothing when nothing changed', async () => {
      expect(await update(OWNER_ID)).toEqual({ ok: true, value: { changed: false } })
    ...
    it.each([
      [EDITOR_ID, 'mbr_awa', 'MEMBER_EDIT_FORBIDDEN'],
